...
temp[temp$BCI == "Normal" & temp$nCH == 1 & temp$maxVAF < 0.1,]$CH<-"N_small"
temp[temp$BCI == "Normal" & temp$nCH == 1 & temp$maxVAF >= 0.1,]$CH<-"N_large"
temp[temp$BCI == "Normal" & temp$nCH > 1,]$CH<-"N_double"
temp[temp$BCI != "Normal" & temp$nCH == 0,]$CH<-"I_control"
temp[temp$BCI != "Normal" & temp$nCH == 1 & temp$maxVAF < 0.1,]$CH<-"I_small"
temp[temp$BCI != "Normal" & temp$nCH == 1 & temp$maxVAF >= 0.1,]$CH<-"I_large"
temp[temp$BCI != "Normal" & temp$nCH > 1,]$CH<-"I_double"
...
## cox
cox6 <- coxph(Surv(Year, Status) ~ Age_categ + Sex + EverSmoke + Caucasian + PC1 + PC2 + PC3 + PC4 + PC5 + CH, data = temp)

## Tables
mvdf1<-extractCox(cox6,"Lymphoid",c("CHN_small","CHN_large","CHN_double"),c("CH, VAF<0.1","CH, VAF>=0.1",">1 CH"),"Any")
...
pdf(paste(figDir,"scripts/Final_figs/Fig_S5_mCA_CBC.pdf",sep=""),width=11,height=6)
print (P)
dev.off()
